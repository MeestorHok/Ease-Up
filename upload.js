var EaseUp = new (function () {
  ...
  self.addUnit = function (thisUnit) {  // add a unit to the hierarchy
      ...
      unit.innerHTML = '<div style="display: inline-block; padding-right: 10px; padding-top: 10px"><a href="javascript:void(0);" onclick="removeNode(this)"><i class="fa fa-times text-danger"></i></a> Unit ' +
          nextNum + ': <input type="text" class="h-unit-input squared-off" placeholder="Unit Name Here" name="unit-' + thisNum + '" onkeyup="displayDescription(this, \'unit\', ' + thisNum + ')" /></div>' +
          ...
          thisNum + '" accept="image/x-png, image/jpeg, video/mp4" class="inputFile" onchange="changeInputFiles(this, \'unit-' + thisNum + '\')" name="file-unit-' + thisNum + '" style="display: none"></a>' +
      ...
      topic.innerHTML = '&nbsp&nbsp&nbsp&nbsp<a href="javascript:void(0);" onclick="addTopic(this)">Add Topic</a>';
      ...
      newUnit.innerHTML = '<a href="javascript:void(0);" onclick="addUnit(this)">Add Unit</a>';
  ...
  self.addTopic = function (thisTopic) {  // add a topic to the unit within a hierarchy
      ...
      topic.innerHTML = '<div style="display: inline-block; padding-right: 10px; padding-top: 10px">&nbsp&nbsp&nbsp&nbsp<a href="javascript:void(0);" onclick="removeNode(this)"><i class="fa fa-times text-danger"></i></a> Topic '+
          nextNum +': <input type="text" class="h-topic-input squared-off" placeholder="Topic Name Here" name="topic-' + bothNums + '" onkeyup="displayDescription(this, \'topic\', \'' + bothNums + '\')" /></div>' +
          ...
          bothNums + '" accept="image/x-png, image/jpeg, video/mp4" class="inputFile" onchange="changeInputFiles(this, \'topic-' + bothNums + '\')" name="file-topic-' + bothNums + '" style="display: none"></a>' +
      ...
      newTopic.innerHTML = '&nbsp&nbsp&nbsp&nbsp<a href="javascript:void(0);" onclick="addTopic(this)">Add Topic</a>';
          ...
          if (node.nodeType == 1) {
              if(node.getAttribute('unit') != null){
                  ...
                  node.firstChild.innerHTML = '&nbsp&nbsp&nbsp&nbsp<a href="javascript:void(0);" onclick="removeNode(this)"><i class="fa fa-times text-danger"></i></a> Topic '+
                      (num - 2) +': <input type="text" class="h-topic-input squared-off" placeholder="Topic Name Here" name="topic-' + bothNums + '" value="' + oldValue + '" onkeyup="displayDescription(this, \'topic\', \'' + bothNums + '\')" />';
              }
              num += 1;
          }
      }
...
/******************************CREATE HIERARCHY FILE SYSTEM********************************/
  ...
  self.uploadFile = function (parentID) {  // if user clicks upload button
      ...
      if (fileSizeCheck == 'true' && fileTypeCheck == 'true') { // if error handling is good
          ...
          ajaxStarted += 1;
          // rename file
          newFilename = 'uploads/' + renameFile(file);
          ...
          request.addEventListener('readystatechange', function(e){
             if(this.readyState == 4) { // all's good
                  if (this.status == 200) { // successfully uploaded
                      ...
                      throwError(feedbackID, 'Upload failed and I don\'t know why! Please let us know about this.');
                      mustStay = false;
                  }
              }
          });
  ...
  self.changeInputFiles = function (thisInput, thisNum) {  // if a file is selected or deselected
      ...
      if ($.inArray(file_ext, allowed) !== -1) {
          ...
          fileTypeCheck = false;
          throwError(thisFeedbackID, 'That filetype is not allowed.');
      }
      ...
      if (totalSize > 16000000) { // if file size is too large
          fileSizeCheck = false;
          throwError(thisFeedbackID, 'File size is too large.');
      } else {
          fileSizeCheck = true;
      }
...
self.createInformation = function (hierarchy) {  // retrieve hierarchy information
    ...
    for(var i = 0; i < parent.childNodes.length - 1; i++) { // foreach unit
        ...
        if (unit.nodeType == 1 && unit.firstChild.lastChild.value != null && unit.firstChild.lastChild.value != "") {
            ...
            var uFilename = uploadFile(unit.getAttribute('id').toString());
            ...
            for(var n = 3; n < unit.childNodes.length - 1; n++) { // foreach topic
                ...
                if (topic.nodeType == 1 && topic.firstChild.lastChild.value != null && topic.firstChild.lastChild.value != "") {
                    ...
                    var tFilename = uploadFile(topic.getAttribute('id').toString());
...
self.createLesson = function () {  // finally attempt to create lesson
    ...
    if (title == '' || description == '') { // if a field is empty
        throwError('feedback', '<p class="alert alert-danger squared-off">Your lesson must have a title and a description.</p>');
        return;
    } else {
        var information = createInformation('hierarchy');
        clearError('feedback');
        // this will be used to upload lesson to the database
        uploadLesson(title, description, information);
    }
};
...
self.uploadLesson = function (title, description, information) {  // send AJAX request to push upload to database
    if (ajaxStarted == ajaxFinished /*uploads are finished*/) {
        $.ajax({
            url : "/create-lesson.php", //URL to the create lesson php script
            type : "POST",
            dataType: "json",
            data: {
                titlePost: title,
